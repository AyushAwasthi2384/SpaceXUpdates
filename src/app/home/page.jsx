...
//             setIsLoading(true)
//             let query = `https://api.spacexdata.com/v3/launches?limit=${launchesPerPage}&offset=${(currentPage - 1) * launchesPerPage}${search && `&mission_name=${search}`}${year && `&launch_year=${year}`}${status && `&launch_success=${status === 'successful'}`}`

//             const response = await fetch(query)
//             const data = await response.json()
//             setLaunches(data)
//             setTotalPages(Math.ceil(110 / launchesPerPage)) // SpaceX API v3 doesn't provide total count, so we use an estimate
//             setIsLoading(false)
//         }

//         fetchLaunches()
//     }, [currentPage, search, year, status])
...
//     return (
//         <div>
//             <div className="flex justify-between mb-4 p-[2rem]">
//                 <input
//                     type="text"
...
//                 />
//                 <select value={year} onChange={(e) => setYear(e.target.value)} className="p-2 rounded-lg border text-black border-gray-300 ml-2">
//                     <option value="">Select Year</option>
//                     {Array.from({ length: new Date().getFullYear() - 2005 + 1 }, (_, i) => 2006 + i).map((year) => (
//                         <option key={year} value={year}>{year}</option>
//                     ))}
//                 </select>
//                 <select value={status} onChange={(e) => setStatus(e.target.value)} className="p-2 text-black rounded-lg border border-gray-300 ml-2">
